...
public class StockRepository {
    ...
    private static List<StockData> initializeStockData() {
        ...
        // try (CSVReader reader = new CSVReader(new FileReader("D:\\MS\\Fall 2024\\Software Engineering\\github\\hw4\\JPMC.csv"))) {
    ...
    private static boolean saveStockData(StockData stockData) {
        ...
        // try (FileWriter writer = new FileWriter("D:\\MS\\Fall 2024\\Software Engineering\\github\\hw4\\JPMC.csv", true)) {
    ...
    private static boolean saveStockDataList() {
        ...
        // try (FileWriter writer = new FileWriter("D:\\MS\\Fall 2024\\Software Engineering\\github\\hw4\\JPMC.csv")) {
    ...
    public List<StockData> findByDateRange(String startDate, String endDate) {
        return stockDataList.stream()
                .filter(stockData -> stockData.getDate().compareTo(startDate) >= 0 &&
                                     stockData.getDate().compareTo(endDate) <= 0)
                .collect(Collectors.toList());
    }
    public StockData findByDate(String date) {
        return stockDataList.stream()
                .filter(stockData -> stockData.getDate().equals(date))
                .findFirst()
                .orElse(null);
    }
}